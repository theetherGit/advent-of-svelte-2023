<script lang="ts">
    ...
    export let options = [] as typeof tags;
    ...
    <Popover.Trigger asChild let:builder>
        <Button builders={[builder]} variant="outline" size="sm" class="h-8 border-dashed">
            ...
            {#if filterValues && filterValues.length > 0}
                ...
                <div transition:fly class="hidden space-x-1 lg:flex">
                    ...
                    {:else}
                        {#each filterValues as option}
                            <Badge variant="secondary" class="rounded-sm px-1.5 py-0.5 font-normal capitalize">
                                {option}
                            </Badge>
                        {/each}
                    {/if}
        ...
        <Command.Root>
            ...
            <Command.List>
                ...
                {#if filterValues && filterValues.length > 0}
                    <Command.Separator />
                    <Command.Item
                            class="justify-center text-center rounded-t-none"
                            onSelect={() => {
							filterValues = [];
						}}
                    >
